...
## Current conventions (updated)
- **Hash routing** (`createHashRouter`) and `vite base: './'`. Public files are referenced **without** a leading slash (use `pub()` from `src/lib/utils.ts`), so the build runs under any path.
- **No runtime face AI on demo media.** Photo boxes and video tracks are precomputed ([MEDIA_PIPELINE.md](MEDIA_PIPELINE.md)). The live detector (`src/media/face.ts`) is used only by the Upload screen for new photos and by the video studio for local files opened by the user.
- **Seed invariants:** 1,512 students, 1,388 families, 1,271 set up; one name per face per photo; names are gender-matched.
- Bump `DATA_VERSION` whenever seed logic or the manifest changes.
